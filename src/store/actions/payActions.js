...
export const pay = (price, videoId) =>{
    return async (dispatch, getState) =>{
        ...
        try{
            const result = await axios.post('https://www.avayejaan.ir/api/pay', { price: price, videoId: videoId, userToken: userInfo.token }, {headers: {Authorization: `Bearer ${userInfo.token}`}});
...
export const getPaymentStatusAction = (status, order_id, payId) => {
    return async dispatch =>{
        ...
        try{
            const {data} = await axios.post('https://www.avayejaan.ir/api/pay/status', { status: status, order_id: order_id, payId: payId});
