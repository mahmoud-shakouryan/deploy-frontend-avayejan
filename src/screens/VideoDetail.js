...
const VideoDetail = () => {
  ...
  return (
        ...
        <div  className=" w-screen h-screen fixed top-10 pt-5 flex flex-col items-center justify-start gap-2 bg-theWhite">
                ...
                <p className=" font-secondFont text-xs bg-theWhite font-bold p-1 rounded-md border border-dark">{category}</p>
                ...
                <div className="w-full  flex items-center justify-center gap-8 text-xs sm:mt-2">
                    ...
                    <span className="flex flex-col  font-bold"><span className="text-center"><PaidOutlinedIcon/></span><span className="text-center flex gap-2"><span className="font-secondFont">تومان</span><span>{TOMAN.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</span></span></span>
                    <span className="flex flex-col  font-bold"><span className="text-center"><SdStorageIcon/></span><span className="text-center flex flex-row-reverse gap-1 font-secondFont"><span>{ vol }</span><span>مگابایت</span></span></span>
                    <span className="flex flex-col font-bold"><span className="text-center"><StorageIcon/></span><span className="text-center flex flex-row-reverse gap-1 font-secondFont"><span>{ parts }</span><span>فایل</span></span></span>
                </div>
                <div className=" w-full flex items-center justify-center gap-10 text-xs font-firstFont sm:mt-10">
                    <Link to='/videos?page=1' className="bg-orange p-2 rounded shadow-dark shadow-sm w-2/5 sm:w-40 h-10"><button className="w-full flex items-center justify-between"><span><ArrowBackIcon/></span><span className="font-bold">برگشت به ویدیوها</span> </button></Link>
                    <button onClick={addToCardHandler} className="bg-orange p-2 rounded shadow-dark shadow-sm w-2/5 sm:w-40 h-10 flex items-center justify-between"><span><AddShoppingCartIcon/></span><span className="font-bold">افزودن به سبد خرید</span></button>
                </div>
